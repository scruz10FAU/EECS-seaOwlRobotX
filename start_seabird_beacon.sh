#!/bin/bash
# start_seabird_beacon.sh — Seabird mission launcher using the YOLO + HSV
# beacon detector (beacon_detector_config.py) instead of the HSV-only buoy_detector.py.
#
# Run this script from the directory containing all the component scripts.
#
# Usage:
...
#   DETECTOR_ARGS="--display" ./start_seabird_beacon.sh               # OpenCV preview
#   MUTE=SWEEP ./start_seabird_beacon.sh                              # hide SWEEP output
#   RECORDER_ARGS="--save-every 3" ./start_seabird_beacon.sh
#
# Config files (edit to change topics / models / thresholds):
#   beacon_config_physical.json  — physical drone (/hires_front_small_color + /tof_depth)
#   beacon_config_sim.json       — Isaac Sim front camera (/iris_0/front_cam/*)
#
...
#   Physical: drone hardware on + modal camera driver running
...
# Default DETECTOR_ARGS points at the sim config; override on the command line
# for physical drone use.
DETECTOR_ARGS="${DETECTOR_ARGS:---config beacon_config_sim.json}"
...
# subscriptions before the next one starts.
START_DELAY=1.5
...
spawn_child() {
    ...
    # Per-component extra args via <TAG>_ARGS env var (e.g. DETECTOR_ARGS).
...
echo "──────────────────────────────────────────────────────────"
echo "  All nodes started. Streaming output (Ctrl-C to stop)."
echo "──────────────────────────────────────────────────────────"
echo ""
